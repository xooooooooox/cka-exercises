...
The reference solution often includes verification / observation commands after the main action (e.g. \`kubectl get\`, \`kubectl describe\`, \`kubectl logs\`, \`kubectl get --raw='/healthz'\`, etc.). Those are illustrative — they show the author confirming the resulting cluster state, NOT additional requirements. Unless the task body explicitly asks the student to verify / observe / check / show the result, do NOT mark an answer down for omitting these follow-up reads. Grade the answer on whether it ACCOMPLISHES the requested mutation, not on whether it mirrors the reference's debugging trail.
