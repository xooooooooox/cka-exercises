...
### Changed
- **ca-2-004 task explicitly asks for all three health endpoints** — added a task-body paragraph and ℹ️ info note to "Get the status of the control plane components (cluster health)" so the scope is unambiguous (was just a one-line title; LLM grader was marking partial-credit answers down even though they were defensible interpretations of the ambiguous original). The ℹ️ note also documents the modern `/livez` + `/readyz` split vs. the legacy `/healthz` endpoint. (this commit)
