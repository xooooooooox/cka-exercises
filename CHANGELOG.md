...
### Fixed
- **Fullscreen quizbar utility row now matches the grade row + regular Quiz controls** — `.qbar-util-row` switched from `display: flex; flex-wrap: wrap` (buttons sized to content) to `display: grid; grid-template-columns: repeat(6, 1fr)`, so the six buttons (📋 Questions / Prev / Flag / Reveal / Skip / Next) sit in 6 equal columns full-stretch just like the 3-col grade row above and the 5-col regular Quiz utility row. (this commit)
...
### Added
- **`desktop-quiz-setup.png` — new Quiz configure-form screenshot** added to `assets/screenshots/`, embedded inline at the "Setup screen" paragraph of WEBAPP_GUIDE §2 🎯 Quiz (EN + CN) and appended to the README desktop screenshot strip. Build-script mirror list grows to 5 entries; spec table updated. (this commit)
...
### Changed
- **ca-2-004 task scope clarified via standard `**Task:**` heading** — "Get the status of the control plane components (cluster health)" now states explicitly that all three of `/livez`, `/readyz`, `/healthz` should be queried with `?verbose`. Replaces the earlier c8896e8 attempt that used raw prose without the conventional `**Task:**` heading and dumped a teaching-style ℹ️ note explaining each endpoint's semantics into the question body — the ℹ️ block is gone now (concept teaching belongs to the linked docs + reference solution, not the prompt). (this commit)
