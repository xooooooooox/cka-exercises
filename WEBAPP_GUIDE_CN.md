...
> - **自动同步 Gist。** 配置好 Gist 后，最后一次编辑 30 秒后自动 Push。再加上 pre-pull backup + 推送冲突检测，多设备协作不再丢数据。
...
**Pre-pull backup + 一键回滚。** 每次 Pull（或 Backup 文件 Import）前，SPA 会先把当前状态快照到 `cka:sync:prepull-backup`，保证 import 可逆。如果 Pull 把你想留的内容覆盖了，去 Settings → Sync 点 **↩ Restore pre-pull backup**（只有当存在 backup 时才会显示这个按钮）—— 一键确认 → 状态恢复 → 页面 reload → backup 被消费掉。

**Push 推送冲突检测。** Push 前 SPA 会先拿一次 gist 的 `updated_at`，跟本地"上次同步"基线比对。如果 gist 在你上次同步之后被别处修改过（另一台设备 Push 了 / 你在 github.com 改了），会弹确认对话框，标明两边时间戳：**OK** = 强制 Push（覆盖远端改动），**Cancel** = 停手让你先 ⬇ Pull 手动合并。第一次 Push 没有基线，跳过这个检查直接推。

**关页面安全网。** 如果你在 30 秒 debounce 窗口内关掉浏览器，会有一次 best-effort 的 `keepalive:true` PATCH 在 `beforeunload` 时触发 — 不让你最后一波编辑丢失。
