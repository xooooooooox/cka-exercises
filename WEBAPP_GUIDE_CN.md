...
- **🐞 Mark for follow-up**（每张卡片 ⭐ Bookmark 旁边）—— 点击弹一个 **scope 选择器**：🔧 Solution issue / 📝 Task issue / 🔧📝 Both。挑出哪边有问题，按钮就显示对应配色（红边框 = Solution，蓝边框 = Task，红蓝渐变 = Both）。完全不弹表单，不打开 GitHub。同一项再点 = toggle off；"🗑 Unflag all" = 全清。全屏模式下答题框标签行也有同款 🐞 按钮，跟 ⛶ / 🔧 / 📝 / 💡 同排显示。
- **🐛 Suggest a fix** + **💾 Save draft** —— 部分填写弹窗，存为草稿（数据保留，之后再编辑完）。
- **🐛 Suggest a fix** + **🚀 Open GitHub issue** —— 立即提交，并在队列里留一条 "Already opened" 记录。

之所以用 🐞 瓢虫而不是 🚩，是为了跟 Quiz 模式里"标记本题待复习"的 🚩 Flag 按钮在视觉上明确区分（两者含义完全不同）。如果一道题同时打了 Solution + Task 两个标，header 🐛 popover 里会显示为**两条独立条目**（一条 `answer-fix` tag、一条 `task-fix` tag），各自可以 Edit / Open / Remove。
