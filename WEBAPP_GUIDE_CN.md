...
**暂存到队列稍后批量提交**：刷题中途发现问题不想立刻打断节奏？在报告弹窗里点 **💾 Save draft** —— 报告会被存到本地队列，不会立即打开 GitHub。打开 **Settings → 🐛 Issues** 就能看到全部已暂存的报告（answer-fix 和 task-fix 一起列出）。每条三个按钮：**✏ Edit** 重新打开弹窗带 draft 预填、**🚀 Open** 单独跳到 `github.com/issues/new`、**🗑 Remove** 把这条草稿丢掉。底部还有一个 **🚀 Open all on GitHub** 批量打开 —— 每条间隔 150ms 单独开 tab，避免被弹窗拦截器一次性拒绝。所有已暂存报告通过 Gist 同步到所有设备 —— 在 iPhone 上记下的问题，在 Mac 上也能看到。
