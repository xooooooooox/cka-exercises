...
- **🚩 Flag**（每张卡片 ⭐ Bookmark 旁边）—— 一键标记 "这道题之后再看"，**完全不弹任何表单**，直接进队列。
...
- **To submit** —— flag 的题 + 你还没去 GitHub 开 issue 的草稿。每行按钮：**📝 Write report** / **✏ Edit**（打开弹窗）、**🚀 Open**（跳 GitHub，自动写 `submittedAt`）、**🗑 Remove**。
...
Settings → 🐛 Issues 也保留了同一份列表，宽屏视图（适合一次性清理大量积压）。所有队列项通过 Gist 跨设备同步。
