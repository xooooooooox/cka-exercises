...
**让 provider 变成活跃（用来批改的）：** 点击 provider 的单选按钮（表单视图会切到该 provider 已保存的 API key + model），然后点 **💾 Save & set as active** — 把表单写到当前选中 provider 的槽位 **并** 把这个 provider 设为活跃。状态行会确认是哪个 provider 刚被激活。
...
**Auto-PR 工作流（维护者一次性设置）**：仓库里有一个 `.github/workflows/answer-fix-pr.yml` 工作流，可以把 `answer-fix` issue 通过 [aider](https://aider.chat) + 你选的 LLM（Anthropic / OpenAI / DeepSeek / Qwen / Doubao / Ollama / Copilot via GitHub Models）转换成 draft PR。**第一次跑之前需要先打开**：**Settings → Actions → General → Workflow permissions → "Allow GitHub Actions to create and approve pull requests"** — 否则工作流会跑完所有步骤、推上修复分支，但最后一步 `gh pr create` 会被 GitHub 拒绝；机器人会在 issue 上留 🚨 评论，并给出 `pull/new/<branch>` 的一键手动建 PR 链接。默认 provider 是 `copilot`（零密钥，复用工作流自带的 `GITHUB_TOKEN` + `models: read` 权限）；要用其它 provider 在仓库 Secrets 里加对应的 `*_API_KEY` 即可。
