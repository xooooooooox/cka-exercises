...
**🐛 觉得参考答案有问题？** 个别题目的参考解答把验证命令（`kubectl auth can-i …`）和真正的答案塞在了同一个代码块，结果 LLM 批改时会把正确答案判为 partial、给个 70 分。当批改分数 < 100 时，verdict 下方会出现 **🐛 Reference solution looks wrong? Report this mismatch** 链接 — 点开后弹出预填好的反馈窗口（含题目 ID、当前参考、你的答案、LLM 批改细节），结尾的 **🚀 Open GitHub issue** 会在新标签页里把 GitHub Issue 表单填好；也可以 **📋 Copy as markdown** 拷贝到剪贴板。没批改的情况下，展开解答后点 **🐛 Suggest a fix** 同样可以反馈。草稿保存在本地（`cka:fix-draft:<id>`），可以慢慢改后再提交；Export / Gist Push 会带上。
