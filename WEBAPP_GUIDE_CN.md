...
**每个 provider 独立记忆。** 每个 provider 的 API key、model、baseUrl、最近 Test 拿到的 model 列表都是**分开**存的。点不同的 provider 单选按钮就会把表单切到该 provider 已保存的配置 — 从 DeepSeek 切到 OpenAI 不会覆盖你的 DeepSeek key。标题旁会显示 "N of 6 configured"；已配置的 provider 卡片显示绿色 ✓，当前活跃的还会有 ★。点 Save 总是会把当前选中的 provider 设为活跃。
