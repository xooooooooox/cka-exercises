...
- **Prose concision**: docs explain *non-obvious* things only. Don't pad with how-to-`cp` / how-to-`git add` / how-to-rename boilerplate, don't add closing lines that just restate what the heading already said, don't write "this is a placeholder until X" pointers when the placeholder is visually obvious. If a step requires only basic shell + git literacy, omit it; if it's genuinely surprising, document it. A sentence beats a paragraph; a bullet list beats a sentence when the items are parallel. Same goes for CHANGELOG entries (see `## Changelog discipline` below for the explicit length target).
