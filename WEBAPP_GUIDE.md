...
**Queue for later.** Don't want to interrupt your study flow to file every issue immediately? Click **💾 Save draft** in the report modal — your report goes to a local queue instead of opening GitHub. Open **Settings → 🐛 Issues** to see every queued report (across both answer-fix and task-fix). Per-row actions: **✏ Edit** reopens the modal with the draft loaded, **🚀 Open** sends the report to `github.com/issues/new`, **🗑 Remove** drops the draft. The **🚀 Open all on GitHub** button at the bottom batch-opens every queued report in separate tabs (with a 150 ms stagger so the popup blocker plays along). Queued reports sync via Gist so the queue is the same across all your devices.
