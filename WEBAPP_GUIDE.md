...
**Auto-PR workflow (maintainer-side, one-time setup).** Reported `answer-fix` issues can be triaged via the **Answer-fix → draft PR** GitHub Actions workflow (`.github/workflows/answer-fix-pr.yml`). It uses [aider](https://aider.chat) + a model of your choice (Anthropic / OpenAI / DeepSeek / Qwen / Doubao / Ollama / Copilot via GitHub Models) to produce a draft PR that closes the issue. Before it can post PRs you need to flip **Settings → Actions → General → Workflow permissions → "Allow GitHub Actions to create and approve pull requests"** once. Without that the workflow runs to completion and pushes the fix branch, but the final `gh pr create` call is rejected by GitHub — the bot will post a 🚨 comment on the issue with a one-click `pull/new/<branch>` recovery link. The default provider is `copilot` (zero-secret, uses the workflow's `GITHUB_TOKEN` via the `models: read` permission); to use the others, add the corresponding `*_API_KEY` secret.
